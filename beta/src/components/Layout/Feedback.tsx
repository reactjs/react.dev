...
export function FeedbackProvider({children}: {children?: ReactNode}) {
  ...
  const [submittedOnPaths] = useState<string[]>([]);
  ...
  const isSubmitted = submittedOnPaths.includes(cleanedPath);

  useLayoutEffect(() => {
    // Reset popup after navigation
    if (previousPath.current !== cleanedPath) {
      previousPath.current = cleanedPath;
      setForceHide(isSubmitted);
      setForceShow(false);
    }
  }, [
  ...
  const hide = useCallback(() => {
    setForceShow(false);
    setForceHide(true);
  }, [setForceShow, setForceHide]);

  const onSubmit = useCallback(() => {
    clearTimeout(timeoutRef.current);
    submittedOnPaths.push(cleanedPath);
    timeoutRef.current = setTimeout(() => {
      hide();
    }, 1000);
  }, [hide, cleanedPath, submittedOnPaths]);
...
export function ButtonShowFeedback() {
  const {forceShow, showOrHide} = useContext(FeedbackContext);
  ...
  return (
    <button
      ...
      className={cn(
        ...
        {
          'bg-card dark:bg-card-dark': forceShow,
        }
      )}
      onClick={showOrHide}>
      {feedbackIcon}
...
export function Feedback({
  ...
  const {forceShow, forceHide, hide} = useContext(FeedbackContext);

  const visible = (visibleByDefault && !forceHide) || forceShow;

  useHideOnClickOutside(popupRef.current, visible && hideOnClickOutside, hide);

  return (
    <div ref={popupRef} className={className(visible)}>
      <SendFeedback />
    </div>
  );
}
...
function useHideOnClickOutside(
  ...
  useEffect(() => {
    ...
    function handleDocumentClickCapture(e: MouseEvent) {
      if (!popup!.contains(e.target as any)) {
        e.stopPropagation();
        e.preventDefault();
        hide();
      }
...
function SendFeedback() {
  const {isSubmitted, onSubmit} = useContext(FeedbackContext);
  return (
