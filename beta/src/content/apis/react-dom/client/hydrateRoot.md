...
You shouldn't need to call `hydrateRoot` again or to call it in more places. From this point on, React will be managing the DOM of your application. If you want to update the UI, your components can do this by [using state.](/hooks/react/useState)
...
It is uncommon to call [`root.render`](#root-render) on a hydrated root. Usually, you'll [update state](/hooks/react/useState) inside one of the components instead.
...
* **optional** `options`: A object contain options for this React root.
  ...
  * `identifierPrefix`: optional prefix React uses for IDs generated by [`useId`.](/hooks/react/useId) Useful to avoid conflicts when using multiple roots on the same page. Must be the same prefix as used on the server.
