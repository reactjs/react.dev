...
* [`onSelect`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement/select_event): An event handler function. Fires after the selection inside the `<input>` changes. React extends the browser `onSelect` event to also fire for empty selection and on edits (which may affect the selection).
