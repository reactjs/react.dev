...
* `onSelect`: An event handler function. Fires after the selection inside the `<input>` changes. Similar to the non-standard browser [`selectionchange` event,](https://developer.mozilla.org/en-US/docs/Web/API/HTMLTextAreaElement/selectionchange_event) but works across different browsers.
