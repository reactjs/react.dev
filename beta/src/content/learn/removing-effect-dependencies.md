...
To fix the issue, don't read `messages` inside the Effect. Instead, pass an [updater function](/hooks/react/useState#updating-state-based-on-the-previous-state) to `setMessages`:
...
To solve this, use the [updater function](/hooks/react/useState#updating-state-based-on-the-previous-state) and write `setCount(c => c + 1)` instead of `setCount(count + 1)`:
