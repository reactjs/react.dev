...
> Check out the [JavaScript Refresher](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions) for more on arrow functions.
...
Built-in components like `<button>` and `<div>` only support [browser event names](/learn/responding-to-events#adding-event-handlers) like `onClick`. However, when you're building your own components, you can name their event handler props any way that you like.
