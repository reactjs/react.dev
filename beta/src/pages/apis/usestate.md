...
* React batches state updates: it does not update the screen until right after all the event handlers have called their `set` functions. This prevents multiple re-renders during a single event. In the rare case that you need to force React to update the screen earlier so that you can access the DOM, you can use [`flushSync`](/apis/flushsync).
  - *In React 17 and earlier,* batching is only enabled for the `set` function calls during React-managed event handlers, like the ones you put in your JSX. React updates the screen at the end of the browser event. Updates from two distinct events never get batched together. 
  - *In React 18 and later,* batching is enabled for all `set` function calls. This includes both React event listeners and listeners you've added manually with `addEventListener`. React updates the screen in the next [microtask](https://developer.mozilla.org/en-US/docs/Web/API/HTML_DOM_API/Microtask_guide). Updates from two distinct *intentional* events like clicks or typing never get batched together. 
