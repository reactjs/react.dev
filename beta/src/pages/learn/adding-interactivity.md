...
React works this way to help you avoid subtle bugs. Here is a little chat app. Try to guess what happens if you press "Send" first and _then_ change the recipient to Bob. Whose name will appear in the `alert` five seconds later?
...
You can fix this by passing an _updater function_ when setting state. Notice how replacing `setScore(score + 1)` with `setScore(s => s + 1)` fixes the "+3" button. This is handy if you need to queue multiple state updates.
