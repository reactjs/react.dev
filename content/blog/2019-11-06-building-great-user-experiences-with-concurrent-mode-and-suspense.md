...
It turns out that this approach has some limitations. Consider a page that shows a social media post by a user, along with comments on that post. That might be structured as a `<Post>` component that renders both the post body and a `<CommentList>` to show the comments. Using the fetch-on-render approach described above to implement this could cause sequential round trips (sometimes referred to as a “waterfall”). First the data for the `<Post>` component would be fetched and then the data for `<CommentList>` would be fetched, increasing the time it takes to show the full page. But there’s an often-overlooked second downside to this approach. If `<Post>` eagerly requires (or imports) the `<CommentList>` component, our app will have to wait to show the post *body* while the code for the *comments* is downloading. We could lazily load `<CommentList>`, but then that would delay fetching comments data and the increase the time to show the full page.

## Render As You Fetch

The fetch-on-render approach is widely used by React apps today and can certainly be used to create great apps. But can we do even better? Let’s step back and consider our goal. In the above `<Post>` example, we’d ideally show the more important content - the post body - as early as possible, *without* negatively impacting the time to show the full page (including comments). Let’s consider the key constraints on any solution - and then look at how we can achieve them:
...
Imagine that we’re about to navigate from a list of a user’s posts to the page for a specific post. We’ll need to download the code for that page - `Post.js` - and also fetch its data. Waiting until we render the component has problems as we saw above - the key is to start fetching code and data for a new view *in the same event handler that triggers showing that view. W*e can either fetch the data within our router - if our router supports preloading data for routes - or in the click event on the link that triggered the navigation. It turns out that the React Router folks are already hard at work on building APIs to support preloading data for routes. But *any* routing framework can implement this idea. 
...
But there’s one thing that’s still missing. We’ve shown how to preload *data* for a route - but what about code? The example above cheated a bit and used React.lazy - but it turns out that React.lazy is, as the name implies, *lazy*. It won’t start downloading code until the lazy component is actually rendered - it’s fetch-on-render for code! But not to fear - the React team is considering APIs that would allow bundle splitting and eager preloading. That would allow a user to pass some form of lazy component to a router, and for the router to trigger loading the code alongside its data.

## Putting It All Together

So to recap, achieving a great loading experience means that we need to start loading code and data as early as possible, but without waiting for all of it to be ready. Parallel data and view trees allow us to load the data for a view in parallel with loading the view (code) itself. Fetching in an event handler means we can start loading data as early as possible, and even optimistically preload a view when we have enough confidence that a user will navigate to it. Loading data incrementally allows us to load important data earlier without delaying the fetching of less important data. And treating code as data - and preloading it with similar APIs - allows us to load it earlier too.

## Using These Patterns

These patterns aren’t just ideas -- we’ve implemented them in Relay Hooks and are using them in production throughout the new [facebook.com](http://facebook.com/). If you’re interested in using or learning more about these patterns, here are some resources:
...
* The [experimental release of Relay Hooks](https://relay.dev/docs/en/experimental/a-guided-tour-of-relay) implements the patterns described here. 
* We’ve implemented two similar examples apps that demonstrate implementing concepts:
