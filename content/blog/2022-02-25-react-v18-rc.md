...
In the future, we'd like to add features to React which would allow a component to unmount and remount without resetting the component state. This will give React better performance out-of-the-box, but requires components to be resilient to effects being mounted and destroyed multiple times in a component without resetting the state.

To help surface these issues, React 18 introduced Strict Effects to Strict Mode. With Strict Effects, React will automatically unmount and remount every component in development, whenever a component mounts, without resetting the component state.
