...
* **Suspense trees are always consistent:** If a component suspends before it's fully added to the tree, React will no longer attempt to add it to the DOM in an inconsistent state, hide it, and fire its effects. Instead, React will throw away the new tree completely, wait for the asynchronous operation to finish, and then retry rendering from scratch. React will render the retry attempt concurrently, and without blocking the browser.
