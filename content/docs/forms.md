...
Since the `value` attribute is set on our form element, the displayed value will always be `value` from `useState`, making the React state the source of truth. Since `handleChange` runs on every keystroke to update the React state, the displayed value will update as the user types.
...
Notice that `value` is initialized in the component's `useState` hook, so that the text area starts off with some text in it.
