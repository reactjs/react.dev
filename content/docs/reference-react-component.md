...
Use `shouldComponentUpdate()` to let React know if a component's output is not affected by the possible differences of new state or props from current. This method is called frequently by its own state or props change or being a child of a rendered component. Because of that, this method may be called even if new state or props are identical to current ones. The behavior of not having `shouldComponentUpdate()` is to re-render and in the vast majority of cases you should rely on that default behavior.
...
If you are confident you want to write it by hand, you may compare `this.props` with `nextProps` and `this.state` with `nextState` and return `false` to tell React the update can be skipped. Note that returning `false` does not prevent child components from re-rendering when *their* state and props changes.

We do not recommend doing deep equality checks or using `JSON.stringify()` in `shouldComponentUpdate()`. It is very inefficient and will harm performance due to `shouldComponentUpdate()`'s frequent invocation tendency.

Currently, if `shouldComponentUpdate()` returns `false`, then [`UNSAFE_componentWillUpdate()`](#unsafe_componentwillupdate), [`render()`](#render), and [`componentDidUpdate()`](#componentdidupdate) will not be invoked. On the other hand, returning `true` causes these to be invoked, and unconditionally doing so is same as the default behavior resulting from not having `shouldComponentUpdate()`. In the future React may treat `shouldComponentUpdate()` as a hint rather than a strict directive, and returning `false` may still result in a re-rendering of the component.
