...
this.setState((currentState, currentProps) => {
  return {counter: currentState.counter + currentProps.step};
});
```

Both `currentState` and `currentProps` received by the updater function are guaranteed to be up-to-date. The output of the updater is shallowly merged with `currentState`.
