...
The `react-dom` package provides DOM-specific methods that can be used on the client and server.
...
Starting in React 18, `react-dom/client` provides methods specific to the client:
...
> Note
>
> These `react-dom` methods are considered legacy. Both `render` and `hydrate` will warn that your app will behave as if it's running React 17 (learn more [here](https://reactjs.org/link/switch-to-createroot)):
>- [`render()`](#render)
>- [`hydrate()`](#hydrate)
