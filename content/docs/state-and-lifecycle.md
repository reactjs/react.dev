...
*(setState())
= When it is called, tells React the updated state. Then, React calls render() with the updated state value.
= Update phase in the Lifecycle. 
4-1) React calls render () with the updated state in this.state.data, thus  UI has the updated state.
