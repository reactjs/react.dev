...
const REACT_COMPILER_ENTRY_REGEX = /(['"])react-compiler\1\s*:\s*\[[^\]]*\]/;
...
function removeExpectedErrorsSegment(source, segment) {
  const escaped = escapeRegExp(segment);
  const leadingPattern = new RegExp(`,\s*${escaped}`);
  if (leadingPattern.test(source)) {
    return source.replace(leadingPattern, '');
  }

  const trailingPattern = new RegExp(`${escaped}\s*,`);
  if (trailingPattern.test(source)) {
    return source.replace(trailingPattern, '');
  }

  return source.replace(segment, '');
}

function normalizeObjectSpacing(text) {
  let result = text;
...
function removeReactCompilerEntry(expectedBlock) {
  if (!REACT_COMPILER_ENTRY_REGEX.test(expectedBlock)) {
    return {
      block: expectedBlock,
      isEmpty: /expectedErrors\s*:\s*\{\s*\}/.test(expectedBlock),
    };
  }

  let updated = expectedBlock;

  updated = updated.replace(
    new RegExp(`,\s*${REACT_COMPILER_ENTRY_REGEX.source}`, 'g'),
    ''
  );
  updated = updated.replace(
    new RegExp(`${REACT_COMPILER_ENTRY_REGEX.source}\s*,`, 'g'),
    ''
  );
  updated = updated.replace(
    new RegExp(REACT_COMPILER_ENTRY_REGEX.source, 'g'),
    ''
  );

  updated = normalizeObjectSpacing(updated);

  const hasOtherEntries = /['"][^'"]+['"]\s*:\s*\[[^\]]*\]/.test(updated);

  return {
    block: hasOtherEntries ? updated : '',
    isEmpty: !hasOtherEntries,
  };
}

function buildCodeFenceLine(lang, metaPortion, errorLines) {
  ...
  const hasObjectWithColon = objectSegment && /:/.test(objectSegment.object);
  ...
  if (hasObjectWithColon) {
    ...
    if (errorLines.length > 0) {
      if (expectedMatch) {
        ...
        updatedObject = object.replace(expectedMatch[0], updatedExpected);
      } else {
      ...
      const {block: updatedBlock, isEmpty} = removeReactCompilerEntry(
        expectedMatch[0]
      );
      if (isEmpty) {
        updatedObject = removeExpectedErrorsSegment(object, expectedMatch[0]);
      } else if (updatedBlock) {
        updatedObject = object.replace(expectedMatch[0], updatedBlock);
      }
    }
    ...
    updatedObject = normalizeObjectSpacing(updatedObject);

    const objectHasContent = /\{\s*[^}\s]/.test(updatedObject);
    const combinedMeta = `${leading}${removeDuplicateExpectedErrorBlocks(
      `${objectHasContent ? updatedObject : ''}${restMeta || ''}`
    )}`.trimEnd();

    if (!combinedMeta.trim()) {
      return `\`\`\`${lang}`;
    }

    const needsLeadingSpace = !combinedMeta.startsWith(' ');
    return `\`\`\`${lang}${
      needsLeadingSpace ? ' ' : ''
    }${combinedMeta.trimStart()}`;
  }
  ...
  if (errorLines.length > 0) {
    ...
    const combined = `${leadingSpace}${metadata}${restMeta || ''}`;
    return `\`\`\`${lang}${removeDuplicateExpectedErrorBlocks(combined)}`;
  }

  let remainingMeta = metaPortion.replace(EXPECTED_ERRORS_BLOCK_REGEX, '');
  remainingMeta = remainingMeta.trimStart();
  ...
  const leadingSpace = metaLeading || ' ';
  return `\`\`\`${lang}${leadingSpace}${remainingMeta}`;
}
...
function getCodeFenceInfo(lines, lineOffsets, block) {
  const fenceIndex = Math.max(block.position.start.line - 1, 0);

  if (fenceIndex >= lines.length) {
    return null;
  }

  const rawLine = lines[fenceIndex];
  ...
  const metaPortion = match[2] || '';
  const occurrences = metaPortion.match(EXPECTED_ERRORS_BLOCK_REGEX) || [];
  const startOffset = lineOffsets[fenceIndex] ?? 0;

  return {
    rawLine,
    lang: match[1],
    metaPortion,
    hasExpected: occurrences.length > 0,
    hasDuplicate: occurrences.length > 1,
    range: [startOffset, startOffset + rawLine.length],
  };
}
    ...
    return {
      Program(node) {
        ...
        for (const block of codeBlocks) {
          ...
          const fenceInfo = getCodeFenceInfo(lines, lineOffsets, block);
          const errorLines = getSortedUniqueNumbers(Array.from(allErrorLines));
          const existingExpectedLines = getSortedUniqueNumbers(expectedErrors);
          const missingExpectedLines = existingExpectedLines.filter(
            (line) => !allErrorLines.has(line)
          );

          const needsUpdate = !arraysEqual(errorLines, existingExpectedLines);
          const replacementLine = fenceInfo
            ? buildCodeFenceLine(
                fenceInfo.lang,
                fenceInfo.metaPortion,
                errorLines
              )
            : null;
          const replacementDiffers =
            fenceInfo && replacementLine !== fenceInfo.rawLine;
          const applyReplacementFix =
            fenceInfo && replacementDiffers
              ? (fixer) =>
                  fixer.replaceTextRange(fenceInfo.range, replacementLine)
              : null;

          const hasDuplicateMetadata = fenceInfo?.hasDuplicate ?? false;
          const hasExpectedErrorsMetadata = fenceInfo?.hasExpected ?? false;
            ...
            const reportData = {
              ...
              loc: {
                start: {
                  ...
                  line: block.position.start.line,
                  column: fenceInfo.rawLine.length,
                },
              },
              message,
            };
