...
function FooterLink({
  ...
  return (
    <NextLink
      href={href}
      className={cn(
        'flex gap-x-4 md:gap-x-6 items-center w-full md:w-[23rem] px-4 md:px-5 py-6 border-2 border-transparent text-base leading-base text-link dark:text-link-dark rounded-lg group focus:text-link dark:focus:text-link-dark focus:bg-highlight focus:border-link dark:focus:bg-highlight-dark dark:focus:border-link-dark focus:border-opacity-100 focus:border-2 hover:bg-gray-5 dark:hover:bg-gray-80',
        {
      ...
      <span className="grid">
        ...
        <span className="block truncate text-lg group-hover:underline">{title}</span>
      </span>
    </NextLink>
  );
}
