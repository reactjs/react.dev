...
* How to combine a reducer with context <br /> | reducer와 context를 결합하는 방법
* How to avoid passing state and dispatch through props <br /> | state와 dispatch 함수를 prop으로 전달하지 않는 방법
* How to keep context and state logic in a separate file <br /> | context와 state 로직을 별도의 파일에서 관리하는 방법

</YouWillLearn>

## Combining a reducer with context | reducer와 context를 결합하기 {/*combining-a-reducer-with-context*/}
...
1. **Create** the context. <br />
| Context를 **생성한다**. 
2. **Put** state and dispatch into context. <br />
| 2. State과 dispatch 함수를 context에 **넣는다**. 
3. **Use** context anywhere in the tree. <br />
| 3. 트리 안에서 context를 **사용한다**.

### Step 1: Create the context | Context 생성하기 {/*step-1-create-the-context*/}
...
- `TasksContext` provides the current list of tasks. <br /> | TasksContext는 현재 tasks 리스트를 제공합니다.
- `TasksDispatchContext` provides the function that lets components dispatch actions. <br /> | `TasksDispatchContext`는 컴포넌트에서 action을 dispatch 하는 함수를 제공합니다.
...
### Step 2: Put state and dispatch into context | State와 dispatch 함수를 context에 넣기 {/*step-2-put-state-and-dispatch-into-context*/}
...
### Step 3: Use context anywhere in the tree | 트리 안에서 context 사용하기 {/*step-3-use-context-anywhere-in-the-tree*/}
...
## Moving all wiring into a single file | 하나의 파일로 합치기 {/*moving-all-wiring-into-a-single-file*/}
...
1. It will manage the state with a reducer. <br /> | Reducer로 state를 관리합니다.
2. It will provide both contexts to components below. <br /> | 두 context를 모두 자식 컴포넌트에 제공합니다.
3. It will [take `children` as a prop](/learn/passing-props-to-a-component#passing-jsx-as-children) so you can pass JSX to it. <br /> | [`children`을 prop으로 받기 때문에](/learn/passing-props-to-a-component#passing-jsx-as-children) JSX를 전달할 수 있습니다.
...
- You can combine reducer with context to let any component read and update state above it. <br /> | Reducer와 context를 결합해서 컴포넌트가 상위 state를 읽고 수정할 수 있도록 할 수 있습니다.
- To provide state and the dispatch function to components below: <br /> | State와 dispatch 함수를 자식 컴포넌트들에 제공하는 방법
  1. Create two contexts (for state and for dispatch functions). <br /> | 두 개의 context를 만듭니다. (각각 state와 dispatch 함수를 위한 것).
  2. Provide both contexts from the component that uses the reducer. <br /> | 하위 컴포넌트들에서 필요한 context를 사용합니다.
  3. Use either context from components that need to read them. <br /> | 하위 컴포넌트들에서 필요한 context를 사용합니다.
- You can further declutter the components by moving all wiring into one file. <br /> | 더 나아가 하나의 파일로 합쳐서 컴포넌트들을 정리할 수 있습니다.
  - You can export a component like `TasksProvider` that provides context. <br /> | Context를 제공하는 `TasksProvider` 같은 컴포넌트를 내보낼 수 있습니다.
  - You can also export custom Hooks like `useTasks` and `useTasksDispatch` to read it. <br /> | 바로 사용할 수 있도록 `useTasks`와 `useTasksDispatch` 같은 사용자 Hook을 내보낼 수 있습니다.
- You can have many context-reducer pairs like this in your app. <br /> | context-reducer 조합을 앱에 여러 개 만들 수 있습니다.
